...
export async function getAllSessionData(): Promise<SessionData> {
  const lastAssessment = await getSessionValue<AssessmentData>('lastAssessment');
  const completionId = await getSessionValue<string>('completionId');
  const processedCompletionId = await getSessionValue<string>('processedCompletionId');
  const conversationComplete = await getSessionValue<boolean>('conversationComplete');
 ...
 */
export async function setConversationComplete(complete: boolean): Promise<void> {
